...
let
  conf = config.modules.battery-check;
in {
  options.modules.battery-check = with lib; {
    enable = mkEnableOption "battery-check";
  };
    ...
    systemd.services.battery-check = {
      ...
      script = ''
        ...
        notify () {
            ...
            -h "int:value:$val" "$status" "$val%, $remaining"
        }
        while true; do
          ...
          elif [[ $status = Charging ]]; then notify
          fi
